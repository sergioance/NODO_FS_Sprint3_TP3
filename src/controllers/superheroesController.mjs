...
export async function agregarSuperheroeController(req, res) {
  try {
    ...
    let { nombreSuperHeroe, nombreReal, edad, planetaOrigen, debilidad, poderes, aliados, enemigos } = req.body;

    // Convertir poderes a array de strings
    poderes = poderes.split(',').map(p => p.trim()).filter(p => p.length > 0);
    aliados = aliados.split(',').map(p => p.trim()).filter(p => p.length > 0);
    enemigos = enemigos.split(',').map(p => p.trim()).filter(p => p.length > 0);
